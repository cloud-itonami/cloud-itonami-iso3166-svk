...
(deftest clean-intake-auto-commits
  (let [[db actor] (fresh)
    ...
    (is (= "Kita Systems SVK" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest fdi-screening-missing-is-held-and-unoverridable
  (testing "missing FDI-screening clearance for a critical foreign investment -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:fdi-screening-missing} (-> (store/ledger db) last :basis)))
