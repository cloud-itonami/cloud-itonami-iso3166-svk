...
2. Register the client's intake: business type, target public function,
   prior filing history in Slovakia if any.
3. Run the advisor in read-only mode against the Slovak Office for
   Public Procurement's tender notices.
4. Compare the checklist against the client's current documentation
   (Obchodný register status, List of Economic Operators registration).
...
- named referral relationship with Slovak-licensed counsel or a
...
a working referral relationship with Slovak-licensed counsel or a
registered agent for whatever licensed representation the law of
Slovakia requires for actual public-procurement filings.
